...
struct State {
    ...
    // Published REAL anchor. Atomic because the CPU-fallback GEN worker reads
    // it while the capture worker advances it after a successful REAL post.
    std::atomic<uint64_t> lastRealSequence{0};
    ...
    // Sequence of the REAL frame immediately before this generation pair.
    // A newly captured (but not yet presented) REAL must not invalidate GEN.
    uint64_t genJobPrevRealSequence = 0;
...
// Adaptive end-to-end latency guard. Generation is optional; REAL frames are not.
// The old fixed 60 ms gate was too conservative for high-FPS sources and, more
// importantly, it treated the output-present interval as extra generation time.
// That double-counted the same cadence budget and caused GEN to be rejected even
// when the GPU had enough headroom. Keep an absolute safety ceiling, but derive
// the normal budget from the measured capture cadence.
constexpr int64_t kHardLatencyBudgetNs = 60'000'000LL;
constexpr int64_t kLatencyReserveNs = 3'000'000LL;
constexpr int64_t kBootstrapGenBudgetNs = 12'000'000LL;

int64_t adaptiveGenerationBudgetNs(int64_t captureIntervalNs) {
    if (captureIntervalNs <= 0) return kBootstrapGenBudgetNs;
    // Leave ~25% of the source interval for capture/present jitter. The budget
    // is never allowed to exceed the absolute latency ceiling.
    const int64_t cadenceBudget = (captureIntervalNs * 75) / 100;
    return std::max<int64_t>(3'000'000LL,
        std::min<int64_t>(cadenceBudget, kHardLatencyBudgetNs - kLatencyReserveNs));
}
...
bool generationFitsHardLatency(const State::PendingFrame &frame,
                               ...
                               size_t outputCount) {
    (void)outputCount;
    if (frame.captureTimestampNs <= 0) return true;
    ...
    // Use the measured source cadence as the primary admission budget. Do not
    // add one output interval per generated frame here: that was effectively
    // charging the presentation cadence twice and was the main reason the old
    // gate reported `estGen=0`/skipped GEN during cold start and then became
    // overly aggressive once history existed.
    const int64_t captureIntervalNs =
        g.emaCaptureIntervalNs.load(std::memory_order_relaxed);
    const int64_t generationBudgetNs = adaptiveGenerationBudgetNs(captureIntervalNs);
    const int64_t estimate = generationEstimateNs > 0
        ? generationEstimateNs
        : kBootstrapGenBudgetNs;

    // CALIBRATION ESCAPE HATCH: until we have at least one REAL measured
    // generation-cost sample, `estimate` above is nothing but the untested
    // kBootstrapGenBudgetNs guess. Gating admission on that guess against a
    // tight cadence-derived budget creates a permanent deadlock in practice:
    // if age-at-decision-time (queueing/scheduling delay, unrelated to actual
    // generation cost) already eats most of the cadence budget, the gate
    // rejects every attempt forever, recordGenerationCostSample() is never
    // called, and the guess never gets replaced by real data — estGen stays
    // pinned at the bootstrap constant across the whole session regardless of
    // resolution. Give an uncalibrated device one generous, bounded window
    // (well inside the absolute latency ceiling) so a first real sample can
    // land; every decision after that uses real telemetry via the normal
    // cadence budget below.
    const bool haveGenerationCostSample =
        g.emaGenNsPerMegapixel.load(std::memory_order_relaxed) > 0 ||
        g.emaGenerationNs.load(std::memory_order_relaxed) > 0;
    // Looser than coldStartLimitNs below because it must also absorb the
    // up-front bootstrap cost estimate, not just cover "no cadence data yet".
    // Still comfortably inside kHardLatencyBudgetNs.
    const int64_t calibrationLimitNs = 40'000'000LL;
    if (!haveGenerationCostSample) {
        return ageNs + estimate <= calibrationLimitNs;
    }

    // Once the frame is already late, only admit generation if its estimated
    // work can still fit in the remaining cadence budget. REAL is never held
    // for this decision.
    // Cold start has no source cadence yet. Give the first pair a small bounded
    // window instead of making age+bootstrapCost impossible to satisfy. The next
    // frames immediately switch to the measured cadence once captureIntervalNs
    // becomes available.
    const int64_t coldStartLimitNs = 20'000'000LL;
    // Steady-state budget (real telemetry available). Widened from a bare
    // single capture interval — which left ~zero slack for ordinary
    // scheduling jitter and starved generation whenever per-frame processing
    // wasn't effectively instantaneous — to a few intervals of headroom,
    // still capped by the absolute ceiling.
    const int64_t cadenceLimit = std::min<int64_t>(
        kHardLatencyBudgetNs - kLatencyReserveNs,
        captureIntervalNs > 0
            ? std::max<int64_t>(generationBudgetNs, captureIntervalNs * 3 + kLatencyReserveNs)
            : coldStartLimitNs);
    return ageNs + estimate <= cadenceLimit;
}
...
int64_t projectGenerationCostNs(uint64_t pixelCount) {
    ...
    const int64_t flat = g.emaGenerationNs.load(std::memory_order_relaxed);
    if (flat > 0) return flat;

    // Cold-start bootstrap. Returning zero made the preflight unable to
    // distinguish “no history yet” from “generation is free”, while another
    // part of the old gate explicitly rejected that zero. Use a small,
    // conservative measured-device-independent budget until the first real
    // completion sample arrives; the budget tightens immediately after that.
    return kBootstrapGenBudgetNs;
}
...
void genWorkerThread() {
    while (true) {
        ...
        uint64_t jobEpoch = 0;
        uint64_t jobPrevRealSequence = 0;
        uint64_t jobPixelCount = 0;
        {
            ...
            jobPrevRealSequence = g.genJobPrevRealSequence;
        ...
        // A newer REAL CAPTURE may arrive while waitIdle() runs. That does NOT
        // make this job stale: the newer capture is still waiting to be
        // presented, so this generation can safely occupy the interval before
        // that REAL. Only a newer PRESENTED REAL invalidates this job.
        const uint64_t currentPresentedReal = g.lastRealSequence.load(std::memory_order_acquire);
        const bool generationStillValid =
            publishOutputs &&
            jobEpoch != 0 &&
            jobPrevRealSequence == currentPresentedReal &&
        ...
        if (generationStillValid) {
            ...
            LOGI("genWorker: dropping stale generated outputs jobEpoch=%llu prevReal=%llu presentedReal=%llu bypass=%d",
                 static_cast<unsigned long long>(jobEpoch),
                 static_cast<unsigned long long>(jobPrevRealSequence),
                 static_cast<unsigned long long>(currentPresentedReal),
...
void workerThread() {
    ...
    while (true) {
        ...
        const uint64_t generationPrevRealSequence = g.lastRealSequence.load(std::memory_order_acquire);
        ...
        auto postCurrentReal = [&]() -> bool {
            ...
            if (posted) {
                ...
                g.lastRealSequence.store(pendingFrame.sequence, std::memory_order_release);
        ...
        if (generationPreflightOk) {
            ...
            const int64_t sourceIntervalNs = captureIntervalNs > 0
                ? captureIntervalNs
                : 0;
            const int64_t safetyWindowNs = adaptiveGenerationBudgetNs(sourceIntervalNs);
            if (genEstimateNs > safetyWindowNs) {
                generationPreflightOk = false;
            }
            // Never start generation when another REAL capture is already waiting.
            // A cold-start timing sample is no longer a reason to reject GEN: the
            // projectGenerationCostNs() bootstrap above supplies a bounded estimate.
            if (queuedBehind > 0) {
                generationPreflightOk = false;
            }
        ...
        if (runFramegen || runAi) {
            ...
            if (useCpuWaitIdleFallback) {
                ...
                g.cpuFallbackGenInFlight.store(true, std::memory_order_release);
                {
                    ...
                    g.genJobPrevRealSequence = generationPrevRealSequence;
            ...
            // Generation belongs to the interval between the PREVIOUS REAL
            // and this REAL. A newer capture may already be queued, but it has
            // not crossed the display boundary yet, so it must not invalidate
            // this generation. The authoritative ordering guard is the last
            // PRESENTED REAL sequence.
            const uint64_t generationEpoch = pendingFrame.captureEpoch;
            const uint64_t presentedRealBeforePair = generationPrevRealSequence;
            const bool generationStillValid =
                generationEpoch != 0 &&
                g.lastRealSequence.load(std::memory_order_acquire) == presentedRealBeforePair &&
                !g.bypass.load(std::memory_order_relaxed) &&
                !g.stopRequested;
            ...
            if (!generationStillValid) {
                ...
                g.pairingResetPending.store(true, std::memory_order_release);
                LOGI("workerThread: stale generation suppressed jobEpoch=%llu prevReal=%llu presentedReal=%llu bypass=%d",
                     static_cast<unsigned long long>(generationEpoch),
                     static_cast<unsigned long long>(generationPrevRealSequence),
                     static_cast<unsigned long long>(g.lastRealSequence.load(std::memory_order_acquire)),
                     g.bypass.load(std::memory_order_relaxed) ? 1 : 0);
                continue;
            }
            ...
            for (size_t outputIndex = 0; outputIndex < g.outputs.size(); ++outputIndex) {
                ...
                // Once GEN has been submitted, do not cancel it merely because
                // the wall-clock budget moved. The compute cost has already been
                // paid and cancelling publication only creates holes in the
                // REAL/GEN sequence. A newer REAL capture, explicit bypass, or
                // shutdown is still a hard invalidation boundary.
                if (currentPresentedReal != generationPrevRealSequence ||
                    g.bypass.load(std::memory_order_relaxed) ||
                    g.stopRequested) {
...
int initRenderLoop(const char *cacheDir, const RenderLoopConfig &cfg) {
    ...
    g.lastRealSequence.store(0, std::memory_order_release);
