    ...
    while (true) {
        ...
        if (runFramegen || runAi) {
            ...
            for (size_t outputIndex = 0; outputIndex < g.outputs.size(); ++outputIndex) {
                ...
                // Only the LAST output of the batch carries the cost sample:
                // that's the one whose completion actually gates when this
                // whole generated pair is fully drained, i.e. the number
                // admission control needs to compare against the capture
                // cadence. Earlier outputs typically finish sooner and would
                // bias the EMA toward an optimistic (too-fast) estimate.
                const bool isLastOutput = (outputIndex + 1 == g.outputs.size());
                if (timedBlit(g.outputs[outputIndex], done, isLastOutput)) postedGeneratedThisIter++;
            }
            postCurrentReal();
